...
int main(void)
{
  ...
  while (1)
  {
	  ...
	  PWM_SetDutyCycle(&htim2, TIM_CHANNEL_1, 90); // 50% PWM on PA0
	  PWM_SetDutyCycle(&htim2, TIM_CHANNEL_2, 0);  // No PWM on PA1
	  HAL_Delay(3000);  // Wait 3 seconds

	  // PA1 Active, PA0 Off
	  PWM_SetDutyCycle(&htim2, TIM_CHANNEL_1, 0);  // No PWM on PA0
	  PWM_SetDutyCycle(&htim2, TIM_CHANNEL_2, 90); // 50% PWM on PA1
	  HAL_Delay(3000);
    /* USER CODE END WHILE */
